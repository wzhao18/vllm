...
def test_eagle_preserves_both_terminal_mamba_replay_boundaries():
    """Exact append and terminal-unit replacement retain usable KDA state."""
    init_none_hash(sha256)
    hash_block_size = 2
    mamba_block_size = 8
    config = KVCacheConfig(
    ...
    for stop in (6, 8, 11):
        allocated = (
            manager.allocate_slots(
                producer, stop - done, num_computed, computed
            )
            if done == 0
            else manager.allocate_slots(producer, stop - done)
        )
        assert allocated is not None
        done = stop
        producer.num_computed_tokens = done
        manager.new_step_starts()
    manager.free(producer)
    manager.new_step_starts()

    replaced_tail = make_request(
        "replaced", [7] * 9 + [9] * 2, hash_block_size, sha256
    )
    _, replaced_hit, _ = manager.get_computed_blocks(replaced_tail)
    assert replaced_hit == 6

    appended = make_request(
        "appended", [7] * 11 + [9], hash_block_size, sha256
    )
    _, appended_hit, _ = manager.get_computed_blocks(appended)
    assert appended_hit == 8
