...
def test_eagle_preserves_append_mamba_replay_boundary():
    """An unchanged prompt followed by a suffix reuses the longest safe state."""
    hash_block_size = 2
    manager = make_full_mamba_manager(
        dcp_world_size=1,
        hash_block_size=hash_block_size,
    ...
    producer = make_request("producer", [7] * 11, hash_block_size, sha256)
    computed, num_computed, _ = manager.get_computed_blocks(producer)
    assert manager.allocate_slots(producer, 8, num_computed, computed) is not None
    ...
    appended = make_request("appended", [7] * 11 + [9], hash_block_size, sha256)
    assert manager.get_computed_blocks(appended)[1] == 8
...
def test_eagle_replay_retention_with_annotated_draft_group():
    ...
    computed_blocks, num_computed, _ = manager.get_computed_blocks(producer)
    assert manager.allocate_slots(producer, 12, num_computed, computed_blocks)
    ...
    consumer = make_request("consumer", prefix + [100, 101], hash_block_size, sha256)
    assert manager.get_computed_blocks(consumer)[1] == 12
    assert offload_boundaries == {12}
...
def test_kimi_dcp_eagle_replay_retains_fine_grained_boundary(prompt_tokens):
    ...
    assert (
        manager.allocate_slots(
            producer, expected_boundary, num_computed, computed_blocks
        )
        is not None
    )
    producer.num_computed_tokens = expected_boundary
    manager.new_step_starts()
    assert (
        manager.allocate_slots(producer, prompt_tokens - expected_boundary) is not None
    )
