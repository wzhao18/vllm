...
Compares FlashInfer fused decode and vLLM's Triton fallback with BF16 recurrent
state. The default shape is Kimi K3 TP8: 12 heads x 128, convolution width 4,
and gate lower bound -5.0. By default, each graph contains independently
allocated state for all 69 KDA layers in the model.

Example:
    .venv/bin/python benchmark_kimi_k3_kda_decode.py \
...
def _bench_graph_layers(calls: list) -> float:
    ...
    total = statistics.median(float(sample) for sample in samples)
...
def main() -> None:
    ...
    print(
        ...
        f"{'fallback BF16 us':>16} {'FI/fallback':>12}"
    )
    for num_heads in args.heads:
        for num_tokens in args.tokens:
            ...
            print(
                ...
                f"{flashinfer_ms * 1e3:>11.2f} "
                f"{fallback_ms * 1e3:>16.2f} "
                f"{fallback_ms / flashinfer_ms:>11.2f}x"
            )
    print(
        "\nFI/fallback is the speedup: values above 1 mean FlashInfer BF16 is "
        "faster. Every measurement replays one CUDA graph "
