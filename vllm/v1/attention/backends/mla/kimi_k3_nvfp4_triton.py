...
# ruff: noqa: E501, SIM102, SIM223, UP045
...
def fp4_mla_repack_v_cache(
    ...
    *,
    ...
    if v_head_dim % block_v != 0:
        raise ValueError(
            f"v_head_dim={v_head_dim} must be divisible by block_v={block_v}."
        )
    if kv_cache.ndim < 5:
        raise ValueError(
...
def _fp4_mla_qk_scores_tile(
    ...
    if FULL_BLOCK_END < Q_HEAD_D:
        ...
        if False and Q_RESIDUAL_D == 64 and TAIL_BLOCK_K == 128:
            ...
            packed_k_cols = (
                non_residual_groups * (FP4_BLOCK // 2) + residual_packed_offsets
            )
            ...
            packed_kr_cols = (
                K_HEAD_D // 2
                + residual_group_offsets * (FP4_BLOCK // 2)
                + byte_offsets_in_group
            )
            ...
            k_vals = tl.where(
                (residual_phase == 2)[None, :], k_residual_vals, k_main_vals
            )
            ...
            q_sf_cols = (
                non_residual_groups
                + residual_sf_cols * 2
                + tl.where(residual_sf_phase == 1, 1, 0)
            )
            k_sf_cols = non_residual_groups + residual_sf_cols
            mask_sf = (virtual_q_sf_cols < Q_HEAD_D // FP4_BLOCK) & (
                residual_sf_phase < 3
            )
            safe_q_sf_cols = tl.where(mask_sf, q_sf_cols, 0)
            safe_k_sf_cols = tl.where(mask_sf, k_sf_cols, 0)
            safe_kr_sf_cols = tl.where(
                mask_sf, K_HEAD_D // FP4_BLOCK + residual_sf_cols, 0
            )
            q_sf_offsets = _fp4_mla_swizzled_sf_offset(
                safe_q_rows[:, None], safe_q_sf_cols[None, :], Q_SF_PER_TOKEN
            )
            ...
            k_scales = tl.where(
                (residual_sf_phase == 2)[None, :], k_residual_scales, k_main_scales
            )
            scores = tl.dot_scaled(
                q_vals,
                q_scales,
    ...
    if ASSUME_FULL_PAGES or page_start < kv_len:
        ...
        if PACK_PROBS:
            ...
            safe_p_rows = (
                p_rows
                if ASSUME_FULL_HEADS
                else tl.where(mask_h, p_rows, p_page * NUM_HEADS)
            ).to(tl.int64)
            ...
            if ASSUME_FULL_HEADS:
                if ASSUME_VALID_PAGES:
                    ...
                    tl.store(
                        p_sf_ptr + sf_offsets, stored_scale, mask=valid_compact_page
                    )
            else:
                tl.store(
                    p_sf_ptr + sf_offsets,
...
def _fp4_mla_attention_page_stats_grouped_kernel(
    ...
    q_sf_offsets = _fp4_mla_swizzled_sf_offset(
        q_rows[:, None], q_sf_cols[None, :], Q_SF_PER_TOKEN
    )
    ...
    for page_rel in tl.range(page_lo, page_hi, num_stages=PAGE_LOOP_STAGES):
        if page_rel < MAX_PAGES:
            ...
            if ASSUME_FULL_PAGES or page_start < kv_len:
                ...
                if ASSUME_VALID_PAGES:
                    physical_page = tl.load(src_page_ids_ptr + compact_page).to(
                        tl.int64
                    )
                    safe_physical_page = physical_page
                else:
                    valid_compact_page = (compact_page >= 0) & (
                ...
                if not ASSUME_VALID_PAGES:
                    k_vals = tl.where(valid_physical_page, k_vals, 0)
                k_scales = tl.load(
                    sf_cache_ptr + safe_physical_page * sf_s0 + k_sf_offsets_main
                )
                scores = tl.dot_scaled(
                    q_vals,
                    q_scales,
                ...
                kt_scales = tl.load(
                    sf_cache_ptr + safe_physical_page * sf_s0 + kt_sf_offsets
                )
                scores = tl.dot_scaled(
                    q_tail_vals,
                    q_tail_scales,
                ...
                if ASSUME_FULL_PAGES:
                    ...
                    valid_t = page_start + offs_t < kv_len
                    scores = tl.where(
                        valid_t[None, :], scores * qk_scale, -float("inf")
                    )
                    page_max = tl.max(scores, axis=1)
                    exp_scores = tl.math.exp2((scores - page_max[:, None]) * _LOG2_E)
                ...
                grouped_probs = tl.reshape(
                    exp_scores, (BLOCK_H, SF_PER_PAGE, FP4_BLOCK)
                )
                amax = tl.max(grouped_probs, axis=2)
                inv_local_scale = tl.where(amax > 0.0, 6.0 / amax, 1.0)
                stored_scale = tl.where(
                ...
                pairs = tl.reshape(
                    scaled_probs, (BLOCK_H, SF_PER_PAGE, FP4_BLOCK // 2, 2)
                )
                even_probs, odd_probs = tl.split(pairs)
                packed = _fp4_e2m1_quantize_packed(even_probs, odd_probs)
...
def _fp4_mla_attention_page_stats_mtp_kernel(
    ...
    for r in tl.static_range(QUERY_LEN_PER_SEQ):
        ...
        if page_start < kv_len_r:
            ...
            q_tail_vals = tl.load(
                q_fp4_ptr
                + q_rows_r[:, None] * q_fp4_s0
                + packed_qt_cols[None, :] * q_fp4_s1
            )
            scores = tl.dot_scaled(
                q_tail_vals,
                q_tail_scales,
...
def _fp4_mla_attention_reduce_stats_kernel(
    ...
    for page_idx in tl.range(0, MAX_PAGES):
        ...
        page_sum = tl.load(
            page_sum_ptr + page_offsets, mask=mask_h & page_valid, other=0.0
        )
...
def _fp4_mla_attention_group_reduce_stats_kernel(
    ...
    for page_idx in tl.range(page_lo, page_hi, num_stages=PIPELINE_STAGES):
        ...
        group_sum = group_sum * tl.math.exp2(
            old_delta * _LOG2_E
        ) + page_sum * tl.math.exp2(new_delta * _LOG2_E)
...
def _fp4_mla_attention_prob_scale_kernel(
    ...
    page_max = tl.load(
        ...
        + page_rel * page_stats_s1
        + safe_offs_h,
        mask=mask_h,
        other=-float("inf"),
    )
    max_score = tl.load(
        max_ptr + query_idx * stats_s0 + safe_offs_h, mask=mask_h, other=0.0
    )
    denom = tl.load(
        denom_ptr + query_idx * stats_s0 + safe_offs_h, mask=mask_h, other=0.0
    )
    factor = tl.where(
        denom > 0.0, tl.math.exp2((page_max - max_score) * _LOG2_E) / denom, 0.0
    )
    ...
    scales = tl.load(p_sf_ptr + sf_offsets, mask=mask_h[:, None], other=1.0).to(
        tl.float32
    )
...
def _fp4_mla_attention_pv_kernel(
    ...
    for page_rel in tl.range(page_lo, page_hi, num_stages=PV_LOOP_STAGES):
        ...
        if ASSUME_FULL_PAGES or page_start < kv_len:
            ...
            if ASSUME_VALID_PAGES:
                safe_compact_page = compact_page
                physical_page = tl.load(src_page_ids_ptr + safe_compact_page).to(
                    tl.int64
                )
                safe_physical_page = physical_page
            else:
            ...
            if USE_PREPACKED_V:
                v_row = (
                    safe_physical_page * (V_HEAD_D // BLOCK_V) + dim_block
                ) * BLOCK_V
                ...
                even_packed = tl.load(
                    ...
                    if ASSUME_VALID_PAGES
                        ...
                        valid_physical_page & mask_v[:, None] & valid_even_t[None, :]
                    ),
                    other=0,
                )
                odd_packed = tl.load(
                ...
                odd_nibble = tl.where(
                    v_use_high_nibble[:, None], odd_packed >> 4, odd_low
                )
    ...
    if PARTIAL_OUT:
        ...
        if ASSUME_FULL_HEADS and ASSUME_FULL_V:
            ...
            tl.store(
                partial_out_ptr + base, acc, mask=mask_h[:, None] & mask_v[None, :]
            )
...
def _fp4_mla_attention_pv_prepacked_v_kernel(
    ...
    if PARTIAL_OUT:
        ...
        tl.store(
            partial_out_ptr + base, out_vals, mask=mask_h[:, None] & mask_v[None, :]
        )
    elif ASSUME_FULL_HEADS and ASSUME_FULL_V:
...
def _fp4_mla_attention_pv_reduce_kernel(
    ...
    for split_idx in tl.static_range(0, PAGE_SPLIT):
        ...
        acc += tl.load(
            partial_ptr + base, mask=mask_h[:, None] & mask_v[None, :], other=0.0
        )
    out_vals = acc * out_scale
    if out_ptr.dtype.element_ty == tl.bfloat16:
        out_vals = out_vals.to(tl.bfloat16)
