...
# ruff: noqa: E501, SIM102, SIM108
"""Triton kernels for MLA FP4 KV-cache helpers.

Adapted from TensorRT-LLM MR 10576 at
``ca5ed123f9310a12ce45b4953aac11c0d1000ad7`` for vLLM page metadata and DCP.
"""
...
@triton.jit
def _fp4_mla_q_sf_offset_h128(
    q_token, q_head_block, head_lane, col_idx, sf_cols: tl.constexpr
):
...
def _fp4_mla_quantize_q_residual_groups(values_even, values_odd):
    ...
    residual_packed, residual_scale_fp8 = _fp4_mla_quantize_q_groups(
...
@triton.jit
def _fp4_mla_quantize_q_kernel(
    q_ptr,
    q_fp4_ptr,
    q_sf_ptr,
    global_scale_ptr,
    ...
    """Quantize ``[latent, RoPE]`` Q and retain a second-order RoPE residual."""
    row = tl.program_id(0)
    group = tl.program_id(1)
    FP4_BLOCK: tl.constexpr = 16
    offsets = tl.arange(0, FP4_BLOCK)
    valid = row < num_rows
    values = tl.load(
        q_ptr + row * q_s0 + (group * FP4_BLOCK + offsets) * q_s1,
        mask=valid,
        other=0.0,
    ).to(tl.float32)
    global_scale = tl.load(global_scale_ptr).to(tl.float32)
    values *= global_scale
    pairs = tl.reshape(values, (FP4_BLOCK // 2, 2))
    even, odd = tl.split(pairs)
    amax = tl.max(tl.maximum(tl.abs(even), tl.abs(odd)), axis=0)
    scale = tl.maximum(amax / 6.0, 0.001953125)
    scale_fp8 = scale.to(tl.float8e4nv)
    inv_scale = _fp4_mla_rcp_approx_ftz(scale_fp8.to(tl.float32))
    packed = _fp4_mla_pack_e2m1x2_rne(even * inv_scale, odd * inv_scale)
    output_group = tl.where(group < 32, group, 32 + 2 * (group - 32))
    byte_offsets = output_group * (FP4_BLOCK // 2) + tl.arange(0, FP4_BLOCK // 2)
    tl.store(q_fp4_ptr + row * 320 + byte_offsets, packed, mask=valid)
    sf_offset = _fp4_mla_q_sf_offset(row, output_group, Q_SF_COLS)
    tl.store(q_sf_ptr + sf_offset, scale_fp8, mask=valid)

    if group >= 32:
        main_even = _fp4_e2m1_to_f32(
            _fp4_e2m1_quantize(even * inv_scale)
        ) * scale_fp8.to(tl.float32)
        main_odd = _fp4_e2m1_to_f32(_fp4_e2m1_quantize(odd * inv_scale)) * scale_fp8.to(
            tl.float32
        )
        residual_even = even - main_even
        residual_odd = odd - main_odd
        residual_amax = tl.max(
            tl.maximum(tl.abs(residual_even), tl.abs(residual_odd)), axis=0
        )
        residual_scale = tl.maximum(residual_amax / 6.0, 0.001953125)
        residual_scale_fp8 = residual_scale.to(tl.float8e4nv)
        residual_inv = _fp4_mla_rcp_approx_ftz(residual_scale_fp8.to(tl.float32))
        residual_packed = _fp4_mla_pack_e2m1x2_rne(
            residual_even * residual_inv, residual_odd * residual_inv
        )
        residual_group = output_group + 1
        residual_offsets = residual_group * (FP4_BLOCK // 2) + tl.arange(
            0, FP4_BLOCK // 2
        )
        tl.store(
            q_fp4_ptr + row * 320 + residual_offsets,
            residual_packed,
            mask=valid,
        )
        residual_sf_offset = _fp4_mla_q_sf_offset(row, residual_group, Q_SF_COLS)
        tl.store(q_sf_ptr + residual_sf_offset, residual_scale_fp8, mask=valid)
...
def _fp4_mla_chunked_cache_gather_kernel(
    ...
    valid_tokens = (
        valid_tokens & (logical_page >= 0) & (logical_page < page_table_stride)
    )
    physical_pages = tl.load(
        page_ids_ptr + page_table_offsets, mask=valid_tokens, other=0
    ).to(tl.int64)
    ...
    if dim_block * FP4_BLOCK >= KV_LORA_RANK:
        ...
        values += (
            _fp4_e2m1_to_f32(residual_nibbles) * residual_scale[:, None] / global_scale
        )
    ...
    if dim_block * FP4_BLOCK < KV_LORA_RANK:
        ...
        tl.store(
            compressed_kv_ptr
            + output_tokens[:, None] * compressed_kv_s0
            + output_dims[None, :],
            values,
...
def _fp4_mla_context_cache_update_kernel(
    ...
    if APPLY_Q_ROPE and work_idx >= NUM_DIM_BLOCKS:
        ...
        q_base = (
            token_idx * q_s0 + q_heads[:, None].to(tl.int64) * q_s1 + Q_NOPE_DIM * q_s2
        )
    ...
    if USE_SLOT_MAPPING:
        storage_position = tl.load(slot_mapping_ptr + metadata_token_idx).to(tl.int64)
    if (storage_position < 0) | (storage_position % HP_BLOCK != 0):
        return

    page_size_i64 = tl.cast(page_size, tl.int64)
    page_idx = storage_position // page_size_i64
    page_pos = storage_position - page_idx * page_size_i64
    if USE_SLOT_MAPPING:
        page_table_offset = tl.full((), 0, tl.int64)
    ...
    if USE_SLOT_MAPPING:
        candidate_slot = tl.load(
            slot_mapping_ptr + safe_candidate_metadata,
            mask=valid_tokens,
            other=-1,
        ).to(tl.int64)
        valid_tokens = valid_tokens & (
            candidate_slot == storage_position + token_offsets
        )
    else:
    ...
    if APPLY_K_ROPE:
        ...
        if dim_block * FP4_BLOCK >= V_HEAD_D:
            rope_dim_mask = (
                mask_even_d & (even_d >= V_HEAD_D) & (odd_d < V_HEAD_D + ROPE_DIM)
            )
            rope_pair_offsets = tl.where(
                rope_dim_mask, (safe_even_d - V_HEAD_D) // 2, 0
            )
            ...
            roped_even, roped_odd = _fp4_mla_rope_fp32(
                even_values, odd_values, cos, sin
            )
    ...
    if STORE_K_RESIDUAL:
        if dim_block * FP4_BLOCK >= HEAD_D - K_RESIDUAL_D:
            quant_scale = (
                stored_scale.to(tl.float8e4nv).to(tl.float32) / kv_global_scale
            )
    ...
    if WRITE_V_PACKED:
        ...
        v_even_rows = ((v_page_offset + physical_page) * V_HEAD_D + logical_v_dims).to(
            tl.int64
        )
        v_cols = page_pos // 2 + tl.arange(0, HP_BLOCK // 2)
        v_even_offsets = (
            v_even_rows[:, None] * v_packed_s0 + v_cols[None, :] * v_packed_s1
        )
    ...
    if STORE_K_RESIDUAL:
        ...
        if dim_block * FP4_BLOCK >= residual_start:
            ...
            residual_low = _fp4_e2m1_quantize(
                residual_even / residual_quant_scale[:, None]
            )
            residual_high = _fp4_e2m1_quantize(
                residual_odd / residual_quant_scale[:, None]
            )
...
def _fp4_mla_generation_q1_kv_tile_update(
    ...
    if WRITE_V_PACKED:
        ...
        v_even_offsets = (
            v_even_rows[:, :, None] * v_packed_s0 + v_cols[None, None, :] * v_packed_s1
        )
        v_odd_offsets = v_even_offsets + v_packed_s0
...
def _fp4_mla_generation_q1_kv_tail8_update(
    ...
    even0 = (
        (pair0_bits & 0xFFFF).to(tl.uint16).to(tl.bfloat16, bitcast=True).to(tl.float32)
    )
    odd0 = (pair0_bits >> 16).to(tl.uint16).to(tl.bfloat16, bitcast=True).to(tl.float32)
    even1 = (
        (pair1_bits & 0xFFFF).to(tl.uint16).to(tl.bfloat16, bitcast=True).to(tl.float32)
    )
    ...
    if WRITE_V_PACKED:
        ...
        v_row_base = (
            (v_page_offset + physical_page) * V_HEAD_D + dim_block * FP4_BLOCK
        ).to(tl.int64)
...
def _fp4_mla_generation_fused_qk_rope_cache_update_kernel(
    ...
    q1_main_kv_programs: tl.constexpr = (
        V_HEAD_D // FP4_BLOCK // Q1_KV_BLOCKS_PER_PROGRAM
    )
    ...
    tl.static_assert(
        ...
        or Q1_KV_BLOCKS_PER_PROGRAM % q1_kv_blocks_per_iteration == 0
    )
    ...
    prompt_offset = tl.load(prompt_offsets_ptr + seq_idx)
    ...
    if FUSE_ROPE_CACHE_STORE:
        ...
        if work_idx >= 0:
            ...
            if q_token_idx < gen_len:
                q_token = seq_idx * QUERY_STRIDE + prompt_offset + q_token_idx
                ...
                if WRITE_Q and q_work_block < q_prefix_programs:
                    if Q_PREFIX_BLOCKS_PER_PROGRAM >= 1:
                        ...
                        for q_prefix_iteration in tl.range(
                            ...
                            prefix_work_block = (
                                q_work_block * Q_PREFIX_BLOCKS_PER_PROGRAM
                                + q_prefix_iteration
                            )
                            ...
                            prefix_packed_words, prefix_scales = (
                                _fp4_mla_quantize_q_groups_words(
                                    prefix_even,
                                    prefix_odd,
                                    BLOCK_Q_HEADS,
                                    prefix_groups,
                            ...
                            prefix_group_offsets = (
                                prefix_work_block * prefix_groups
                                + tl.arange(0, prefix_groups)
                            )
                    ...
                    if USE_EXTERNAL_ROPE_POSITIONS:
                        position = tl.load(rope_positions_ptr + q_token).to(tl.int64)
                    valid_position = position >= 0
                    pair_offsets = tl.arange(0, ROPE_PAIR_BLOCK)
                    pair_mask = pair_offsets < ROPE_DIM // 2
                    q_base = (
                        q_token * q_pe_s0 + head_offsets[:, None].to(tl.int64) * q_pe_s1
                    )
                    q_dims = tl.arange(0, ROPE_DIM)
                    q_dim_mask = (
                        valid_position
                        & row_mask[:, None]
                        & (q_dims[None, :] < ROPE_DIM)
                    )
                    ...
                    if WRITE_Q:
                        ...
                        output_groups = prefix_groups + tl.arange(
                            0, tail_group_count * 2
                        )
    ...
    if FUSE_ROPE_CACHE_STORE and MAX_GEN_TILES == 1:
        if dim_block * FP4_BLOCK >= V_HEAD_D:
            ...
            tail_even = tl.load(
                latent_cache_ptr
                + latent_token * (NUM_DIM_BLOCKS * FP4_BLOCK)
                + tail_even_d,
            ).to(tl.float32)
            tail_odd = tl.load(
                latent_cache_ptr
            ...
            packed_cols = (
                V_HEAD_D // 2
                + group_offsets[:, None] * (FP4_BLOCK // 2)
                + bytes_in_group[None, :]
            )
            kv_base = physical_page * kv_s0
            tl.store(
    ...
    for tile_iter in tl.static_range(0, MAX_GEN_TILES):
        ...
        if FUSE_ROPE_CACHE_STORE and APPLY_ROPE and not q1_shared_main:
            if dim_block * FP4_BLOCK >= V_HEAD_D:
                positions = abs_positions.to(tl.int64)
                if USE_EXTERNAL_ROPE_POSITIONS:
                    positions = tl.load(
                        rope_positions_ptr + safe_latent_tokens,
        ...
        if q1_shared_main:
            ...
            token_scale_col = page_pos // HP_BLOCK
            sf_offsets = _fp4_mla_swizzled_sf_offset(
                safe_all_d, token_scale_col, SF_PER_PAGE
            )
            v_sf_base = tl.cast(local_layer, tl.int64) * tl.cast(
                vsf_s0, tl.int64
            ) + physical_page * tl.cast(vsf_s1, tl.int64)
        ...
        if STORE_K_RESIDUAL and not q1_shared_main:
            if dim_block * FP4_BLOCK >= HEAD_D - K_RESIDUAL_D:
                quant_scale = (
                    stored_scale.to(tl.float8e4nv).to(tl.float32) / global_scale
                )
        low = _fp4_e2m1_quantize(even_values / quant_scale[:, None])
        high = _fp4_e2m1_quantize(odd_values / quant_scale[:, None])
        packed = low | (high << 4)
        ...
        k_sf_offsets = _fp4_mla_swizzled_sf_offset(
            page_positions, dim_block, SF_PER_TOKEN
        )
        tl.store(
            sf_cache_ptr + physical_page * sf_s0 + k_sf_offsets,
            stored_scale,
        ...
        if STORE_K_RESIDUAL and not q1_shared_main:
            ...
            if dim_block * FP4_BLOCK >= residual_start:
                ...
                residual_stored_scale = residual_scale * global_scale
                residual_quant_scale = (
                    residual_stored_scale.to(tl.float8e4nv).to(tl.float32)
                    / global_scale
                )
                residual_low = _fp4_e2m1_quantize(
                    residual_even / residual_quant_scale[:, None]
