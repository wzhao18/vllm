...
class KVCacheStoreSendingThread(KVTransferThread):
    ...
    def _maybe_offload_boundary_states(self, req_meta: ReqMeta) -> bool:
        ...
        tail_offloads: dict[int, list[tuple[int, int, int]]] = {}
        ...
        for group_id, block_id, boundary in offloads:
            ...
            if (
                ...
                and boundary not in eagle_tail_boundaries
            ):
                snapshots.append(entry)
            else:
                tail_offloads.setdefault(boundary, []).append(entry)

        puts = self._boundary_snapshot_puts(req_meta, snapshots)
        if self.coord.enable_partial_hash_hits:
            for boundary_offloads in tail_offloads.values():
                puts.extend(self._sub_block_tail_puts(req_meta, boundary_offloads))
